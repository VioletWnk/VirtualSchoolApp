...
class SchoolStorage(Storage):
    ...
    def add(self):
        ...
        school_address = input('Введите адрес:')
    ...
    def get(self):
        self.show()
        school_id = int(input('Введите номер школы, чтобы получить информацию:'))
        school = self.get_by_id(school_id)
        print(f'\nНазвание школы: {school.name}\nАдрес школы: {school.address}\n\n')

    def update(self):
        self.show()
        school_id = int(input('Введите номер школы, чтобы изменить информацию:'))
        school = self.get_by_id(school_id)
